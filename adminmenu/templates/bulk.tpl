<form method='post'>
    <fieldset>
        ...
        <div class='form-group form-row align-items-center'>
            ...
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                <div class='custom-control custom-checkbox'>
                    <input class='custom-control-input' name='filter[active]' id='filter[active]' type='checkbox'
                           checked />
                </div>
            </div>
        </div>
            ...
            <span class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                    <select class='custom-select' id='filter[group]' name='filter[group]'>
                        ...
                        {foreach from=$customerGroups item=group}
                            <option value='{$group->getID()}'>{__($group->getName())}</option>
                        {/foreach}
                    </select>
                </span>
        </div>
        ...
        <div class='form-group form-row align-items-center'>
            <label class='col col-sm-4 col-form-label text-sm-right' for='filter[language]'>{__('language')}</label>
            <span class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
    ...
    <fieldset>
        ...
        <div class='form-group form-row align-items-center'>
            ...
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                ...
                <div class='custom-control custom-radio'>
                    <input type='radio' class='custom-control-input' id='msgTypeRcs'
                           name='msgType' value='rcs'>
                    <label class='custom-control-label' for='msgTypeRcs'>
                        {__('RCS')}
                    </label>
                </div>
            </div>
        ...
        <div class='form-group form-row align-items-center' id='wrapFlash'>
            <label class='col col-sm-4 col-form-label text-sm-right' for='flash' data-toggle='tooltip'
                   data-placement='left' title='{__("flashDescription")}'>
                {__('flash')}
            </label>
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                <div class='custom-control custom-checkbox'>
                    <input class='custom-control-input' name='flash' id='flash' type='checkbox' />
        ...
        <div class='form-group form-row align-items-center' id='wrapPerformanceTracking'>
            <label class='col col-sm-4 col-form-label text-sm-right' for='performanceTracking' data-toggle='tooltip'
                   data-placement='left' title='{__("performanceTrackingDescription")}'>
                {__('performanceTracking')}
            </label>
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                <div class='custom-control custom-checkbox'>
                    <input class='custom-control-input' name='performanceTracking' id='performanceTracking'
                           type='checkbox'/>
        ...
        <div class='form-group form-row align-items-center' id='wrapDelay'>
            <label class='col col-sm-4 col-form-label text-sm-right' for='delay' data-toggle='tooltip'
                   data-placement='left' title='{__("delayDescription")}'>
                {__('delay')}
            </label>
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
        ...
        <div class='form-group form-row align-items-center' id='wrapForeignId'>
            <label class='col col-sm-4 col-form-label text-sm-right' for='foreignId' data-toggle='tooltip'
                   data-placement='left' title='{__("foreignIdDescription")}'>
                {__('foreignId')}
            </label>
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                <input class='form-control' maxlength='64' id='foreignId' name='foreignId'/>
            </div>
        </div>

        <div class='form-group form-row align-items-center' id='wrapLabel'>
            <label class='col col-sm-4 col-form-label text-sm-right' for='label' data-toggle='tooltip'
                   data-placement='left' title='{__("labelDescription")}'>
                {__('label')}
            </label>
            <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
        ...
        <div>
            <div class='form-group form-row align-items-center'>
                <label class='col col-sm-4 col-form-label text-sm-right' for='to'>{__('recipients')}</label>
                <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
            ...
            <div class='form-group form-row align-items-center'>
                <label class='col col-sm-4 col-form-label text-sm-right' for='from'>{ucfirst(__('from'))}</label>
                <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
            ...
            <div class='form-group form-row align-items-center'>
                <label class='col col-sm-4 col-form-label text-sm-right' for='text'>{__('text')}</label>
                <div class='col-sm pl-sm-3 pr-sm-5 order-last order-sm-2'>
                    <textarea class='form-control' id='text' name='text' maxlength='1520' required></textarea>
                </div>
            </div>
        </div>
    </fieldset>
    ...
    <div class='save-wrapper'>
        <div class='row'>
            <div class='ml-auto col-sm-6 col-xl-auto submit'>
                <button type='submit' class='btn btn-primary btn-block'>{ucfirst(__('send'))}</button>
            </div>
        </div>
    </div>
</form>
    ...
    Array.from(document.querySelectorAll('input[name=\'msgType\''))
        .forEach(t => t.addEventListener('change', e => {
            ...
            switch (type) {
                case 'rcs':
                case 'voice':
                    smsInputs.forEach(e => e.classList.add('d-none'))
                    break
                case 'sms':
                    smsInputs.forEach(e => e.classList.remove('d-none'))
                    maxLength = 1520
                    break
            }

            text.setAttribute('maxlength', String(maxLength))
        }))
</script>
